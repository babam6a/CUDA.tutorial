...
/**
 * @brief CUDA kernel for naive matrix transpose.
 *
 * This kernel transposes a matrix by switching rows and columns. The 2D matrix is
 * indexed using 1D linear memory with column-major ordering.
 *
...
__global__ void naive_cuda_transpose( const int m, 
                                      ...
                                      double * const c )
{
  /* TODO: Calculate row and column index for the current thread */
	const int myRow = threadIdx.y + blockDim.y * blockIdx.y;
	const int myCol = threadIdx.x + blockDim.x * blockIdx.x;

	/* TODO: Ensure the thread is within the matrix bounds */
	if(INDX(myRow, myCol, m) < m*m )
	{
		/* TODO: Transpose the matrix element by switching row and column indices */
...
int main( int argc, char *argv[] )
{
	...
	*/
	...
	/* call naive cuda transpose function */
