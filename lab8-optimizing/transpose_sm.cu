                                     ...
                                     double * const c )
{
	...
	__shared__ double shared_a[THREADS_PER_BLOCK_Y][THREADS_PER_BLOCK_X];
	...
	if(INDX(myRow, myCol, m) < m * m)
	{
		...
		shared_a[tileY][tileX] = a[INDX(myRow, myCol, m)];
	} 
	...
	if(INDX(myRow, myCol, m) < m * m)
	{
		...
		c[INDX(myCol, myRow, m)] = shared_a[tileY][tileX];
	} 
	return;
